...
(defn find-related-articles
  ...
  (let [results (article-api/related-article-list id {:api-key api-key})
...
(defn runner
  "executes given function, returns it's value or prints any exceptions and returns nil.
  prevents threads from failing silently."
  [f]
  (try
    (f)
    (catch Exception e
  ...
  (let [article-versions (find-article-versions id api-key)
    ...
    (if error?
      ...
      (let [;; article with `id` exists, now find the rest
            ...
            ;; `pmap` to execute functions in parallel
            ...
            recommendations (if (>= num-recommendations 3)
                              ...
                              (let [idx (set (map idx-item recommendations))
                                ...
                                (into recommendations
                                      (take num
                                            (remove #(contains? idx (idx-item %)) recent-articles-with-subject)))))
            ]
        recommendations))))
...
(defn negotiate
  ...
  (let [v2-compatible? true ;; assume whatever content is returned is v1 and v2 compatible
        ...
        ;; todo: traverse all acceptable content types?
...
(defn recommendation-list
  ...
  (let [defaults {:content-type-list api-raml/recommendations-list-content-type
    ...
    (if error?
      ...
      (let [;; we've found recommendations for the given type and id.
